...
現状の実装は、OBJ / MTL / テクスチャ、GLB / GLTF、STLを読み込み、テクスチャ付き表示、ワイヤーフレーム表示、設計図風表示、グリッドフロア付きプレビュー、PNG画像書き出し、WebM / MP4回転動画書き出しを行うデスクトップアプリケーションです。PLY / DAE / FBX / 3MF / USDZなどは後続対応として扱います。
...
| `.png` / `.jpg` / `.jpeg` / `.webp` | テクスチャ | OBJ/MTL、GLTFの外部画像参照として自動解決 |

### 5.3 後続対応形式

| 形式 | 方針 |
|---|---|
| `.ply` | Three.js PLYLoaderで対応検討 |
| `.dae` | ColladaLoaderで対応検討 |
| `.fbx` | FBXLoaderで対応検討。マテリアル再現は限定対応 |
| `.3mf` | 3MFLoaderで対応検討 |
| `.usdz` | Web環境制約を踏まえて検証扱い |
| `.step` / `.stp` | OpenCascade系または外部変換で対応検討 |
| `.blend` | Blender fallbackでGLBへ変換 |
...
FBXは現状未対応です。将来対応時も仕様が複雑で、外部テクスチャ参照やマテリアル再現に差が出やすいため、「読み込めるが完全再現は保証しない」扱いにします。

### 29.2 USDZ対応

USDZは現状未対応です。Web環境での扱いに制約があるため、将来対応時も検証扱いにし、GLB変換ルートも検討します。
...
現状はElectron + React + TypeScript + Three.jsで、OBJ / MTL / Texture、GLB / GLTF、STLの読み込みと、PNG / 透明PNG / WebM / MP4書き出しに対応しています。今後はPLY / DAE / FBX / 3MF / USDZ、PNG連番、GIF、Blender fallback / Assimp fallbackへ拡張します。
